...
**Phase 2: Content Selection & Refinement (Adhering to Tech Resume Best Practices)**
    ...
    2.  **Work Experience & Projects - The Core:**
        ...
        *   **Content Crafting (for each selected Work Experience & Project):**
            ...
            *   **Strategic Highlighting (using \`<strong>\` tags):** Within bullet points in the \`description\` array, identify and wrap the most critical keywords, skills, or quantifiable results (including previously anonymized figures if they are key achievements) directly aligned with the job description using \`<strong>\` and \`</strong>\` tags. This helps draw the reviewer's attention to the most relevant information. *Use this sparingly for maximum impact.*
                *   *Example:* "Developed a new feature using <strong>React and Node.js</strong>, resulting in a <strong>15% increase in user engagement</strong>."
                *   *Example:* "Managed a <strong>$XX million budget</strong> for a portfolio of <strong>cloud infrastructure projects</strong>." (Here, '$XX million' is an anonymized figure being highlighted.)
                *   *Example:* "Reduced API latency by <strong>30%</strong> by optimizing database queries and implementing <strong>caching mechanisms</strong>."
            *   **PAR/STAR/Result-First Structure:** Structure bullet points effectively:
                *   **PAR (Problem-Action-Result):** "Reduced customer support response time by <strong>20%</strong> (Result) by implementing a new ticketing system (Action) to address slow response issues (Problem)." (Often written Action-Problem-Result or Action-Result for brevity).
                *   **Action Verb + Skill + Result (Common for Tech):** "Developed a user-friendly website using <strong>React, Node.js, and PostgreSQL</strong> (Action/Skill), resulting in a <strong>30% increase in user sign-ups</strong> (Result)."
    ...
    3.  **Summary Section (\`userDetails.summary\`):**
        ...
        *   It should clearly state:
            ...
            *   **Key expertise/skills:** Highlight 2-3 core technical competencies most relevant to the job.
            *   **Major achievement(s) or value proposition:** Briefly mention a key quantifiable achievement or how you deliver value.
...
**Phase 3: Output Generation**
    1.  **Generate the Tailored Resume:** Construct the resume in the specified structured JSON format. The \`description\` fields within \`workExperience\` and \`projects\` should contain strings that may include \`<strong>\` tags for highlighting as instructed.
    2.  **Conciseness & Clarity:** Ensure the language is clear, concise, professional, and free of jargon unless it's industry-standard and relevant to the job description. Avoid "fancy/flowery" language.
    3.  **ATS-Friendliness:** The structured nature and keyword focus will inherently aid ATS compatibility. Ensure standard terminology. While \`<strong>\` tags are for visual emphasis, the core text must remain parseable and keyword-rich for ATS.
...
// interface Skill {
// 	name: string;
// 	category: string; // e.g., "Programming Languages", "Frameworks", "Databases", "Cloud", "Tools"
// }
\`\`\`
